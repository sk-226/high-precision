...
!   This version uses the QXFUN quad precision package (although no special
!   special functions are required here except for gamma).

!   David H Bailey   16 Mar 2023
...
!   included in a "real (qxknd)" and an "external" statement.  Prior to
...
!          of quadrature results; default = 32.
!   ndp2   Secondary ("high") precision in digits; default = ndp1.
...
use qxmodule
implicit none
integer, parameter:: m1 = 40, m2 = 32, ndp1 = 34, ndp2 = 2 * ndp1, neps1 = -ndp1, &
  neps2 = -ndp2, nq1 = 11, nq2 = 12 * 2 ** nq1
real (kind (0.e0_qxknd)), external:: second
real (qxknd), external:: quades, quadss, quadts, fun01, fun02, fun03, fun04, &
  fun05, fun06, fun07, fun08, fun09, fun10, fun11, fun12, fun13, fun14, &
  fun15, fun16, fun17, fun18
integer i
character(32) form32
real (kind(0.e0_qxknd)) tm0, tm1, tm2
real (qxknd) err, errmx
real (qxknd) one, t1, t2, t3, t4, wkes(-1:nq2), xkes(-1:nq2), &
  wkss(-1:nq2), xkss(-1:nq2), wkts(-1:nq2), xkts(-1:nq2), zero
real (qxknd) mppic, mpl02, x1, x2
...
mpl02 = qxlog2 ()
errmx = zero
write (form32, '("(f",i2.2,".",i2.2,")")') m1, m2
...
write (6, 3) tm1 - tm0
write (6, fmt=form32) t1
t2 = (mppic - 2.e0_qxknd + 2.e0_qxknd * mpl02) / 12.e0_qxknd
err = t2 - t1
errmx = max (abs (err), errmx)
write (6, 4) err
...
write (6, 3) tm1 - tm0
write (6, fmt=form32) t1
t2 = 0.5e0_qxknd * (exp (0.5e0_qxknd * mppic) - 1.e0_qxknd)
err = t2 - t1
errmx = max (abs (err), errmx)
write (6, 4) err
...
write (6, 3) tm1 - tm0
write (6, fmt=form32) t1
t2 = 5.e0_qxknd * mppic**2 / 96.e0_qxknd
err = t2 - t1
errmx = max (abs (err), errmx)
write (6, 4) err
...
write (6, 3) tm1 - tm0
write (6, fmt=form32) t1
t2 = 2.e0_qxknd * sqrt (mppic) * gamma (0.75e0_qxknd) / gamma (0.25e0_qxknd)
err = t2 - t1
errmx = max (abs (err), errmx)
write (6, 4) err
...
write (6, 3) tm1 - tm0
write (6, fmt=form32) t1
t2 = 0.5e0_qxknd * mppic * sqrt (2.e0_qxknd)
err = t2 - t1
errmx = max (abs (err), errmx)
write (6, 4) err
...
write (6, 3) tm1 - tm0
write (6, fmt=form32) t1
t2 = sqrt (2.e0_qxknd * mppic / exp (1.e0_qxknd))
err = t2 - t1
errmx = max (abs (err), errmx)
write (6, 4) err
...
t1 = t
fun01 = t1 * log (1.e0_qxknd + t1)
return
end function fun01

real (qxknd) function fun02 (t)
...
t1 = t
t2 = sqrt (2.e0_qxknd + t1**2)
fun04 = atan(t2) / ((1.e0_qxknd + t1**2) * t2)
return
end function fun04

real (qxknd) function fun05 (t)
...
t1 = t
t2 = 1.e0_qxknd - t
fun07 = sqrt (t1) / sqrt (t2 * (1.e0_qxknd + t1))
return
end function fun07

real (qxknd) function fun08 (t)
...
if (t1 < t3) then
  ...
  fun10 = 1.e0_qxknd / sqrt (tan (t2))
endif
return
end function fun10

real (qxknd) function fun11 (t)
...
t1 = t
fun11 = 1.e0_qxknd / (1.e0_qxknd + t1 ** 2)
return
end function fun11

real (qxknd) function fun12 (t)
...
t1 = t
fun13 = exp (-0.5e0_qxknd * t1 ** 2)
return
end function fun13

real (qxknd) function fun14 (t)
...
real (qxknd) function fun15 (t)
use qxmodule
implicit none
real (qxknd), intent(in):: t

fun15 = 1.e0_qxknd / (1.e0_qxknd + t**2)
return
end function fun15

real (qxknd) function fun16 (t)
use qxmodule
implicit none
real (qxknd), intent(in):: t

fun16 = 1.e0_qxknd / (1.e0_qxknd + t**4)
return
end function fun16

real (qxknd) function fun17 (t)
use qxmodule
implicit none
real (qxknd), intent(in):: t

fun17 = exp (-0.5e0_qxknd * t**2)
return
end function fun17

real (qxknd) function fun18 (t)
use qxmodule
implicit none
real (qxknd), intent(in):: t

fun18 = exp (-0.5e0_qxknd * t**2) * cos (t)
return
end function fun18
...
real (qxknd), intent(out)::  wk(-1:nq2), xk(-1:nq2)
integer, parameter:: iprint = 1024, ndebug = 2
integer i, j, k, k1
real (qxknd) eps2, h, p2, t1, t2, t3, t4, u1, u2
 
write (6, 1)
1 format ('initqes: Exp-sinh quadrature initialization')

eps2 = 10.e0_qxknd ** neps2
p2 = 0.5e0_qxknd * qxpi ()
h = 0.5e0_qxknd ** nq1
wk(-1) = real (nq1, qxknd)
...
!   where u1 = pi/2 * sinh (t1) and u2 = pi/2 * cosh (t1)
  ...
  t2 = exp (t1)
  u1 = 0.5e0_qxknd * p2 * (t2 - 1.e0_qxknd / t2)
  u2 = 0.5e0_qxknd * p2 * (t2 + 1.e0_qxknd / t2)
  xk(k) = exp (u1)
  wk(k) = xk(k) * u2

  if (wk(k) * eps2 > 1.e0_qxknd) goto 100
enddo
...
real (qxknd) function quades (fun, x1, nq1, nq2, neps1, wk, xk)
...
use qxmodule
implicit none
real (qxknd), external:: fun
real (qxknd), intent(in):: x1
integer, intent(in):: nq1, nq2, neps1
real (qxknd), intent(in)::  wk(-1:nq2), xk(-1:nq2)
integer, parameter:: izx = 5, m1 = 40, m2 = 32, ndebug = 2
real (kind(0.d0)), external:: dplog10q
integer i, ierror, ip(0:100), iz1, iz2, j, k, k1, k2, n, nds, nqq1
character(32) form32
logical log1
real (kind(0.d0)) d1, d2, d3, d4
real (qxknd) c10, eps1, eps2, epsilon1, err, h, &
  tsum, s1, s2, s3, t1, t2, t3, tw1, tw2, twi1, twi2, twmx 
real (qxknd) xx1, xx2

epsilon1 = 10.e0_qxknd ** neps1
tsum = 0.e0_qxknd
s1 = 0.e0_qxknd
s2 = 0.e0_qxknd
h = 1.e0_qxknd
...
!   Twi1 and twi2 are the final nonzero values of abs(tw1) and abs(tw2).
  ...
  d3 = dplog10q (eps1) - 1.e0_qxknd
  d4 = dplog10q (eps2) - 1.e0_qxknd

  if (k <= 2) then
    err = 1.e0_qxknd
  elseif (d1 .eq. -999999.e0_qxknd) then
    err = 0.e0_qxknd
  else
    err = c10 ** nint (min (0.e0_qxknd, max (d1 ** 2 / d2, 2.e0_qxknd * d1, d3, d4)))
  endif

!   Output current integral approximation and error estimate.

  if (ndebug >= 2) then
    write (6, 2) k, nq1, nint (dplog10q (abs (err)))
2   format ('quades: Iteration',i3,' of',i3,'; est error = 10^',i7, &
      '; approx value =')
    write (6, fmt=form32) s1
  endif
...
!   where u1 = pi/2 * sinh (t1) and u2 = pi/2 * cosh (t1)
  ...
  u2 = 0.5e0_qxknd * p2 * (t2 + 1.e0_qxknd / t2)
  t3 = exp (u1)
  xk(k) = 0.5e0_qxknd * (t3 - 1.e0_qxknd / t3)
  wk(k) = 0.5e0_qxknd * (t3 + 1.e0_qxknd / t3) * u2
...
real (qxknd) function quadss (fun, nq1, nq2, neps1, wk, xk)
...
!   where u1 = pi/2 * cosh (t1), u2 = pi/2 * sinh (t1)
  ...
  u2 = 0.5e0_qxknd * p2 * (t2 - 1.e0_qxknd / t2)
  t3 = exp (u2)
  t4 = 0.5e0_qxknd * (t3 + 1.e0_qxknd / t3)
  xk(k) = 1.e0_qxknd / (t3 * t4)
  wk(k) = u1 / t4 ** 2
...
real (qxknd) function quadts (fun, x1, x2, nq1, nq2, neps1, wk, xk)
...
real (qxknd) c10, eps1, eps2, epsilon1, err, h, &
  tsum, s1, s2, s3, t1, t2, t3, tw1, tw2, twi1, twi2, twmx
real (qxknd) ax, bx, xki, xt1, xx1, xx2

!  These two lines are performed in high precision (nwds2 words).

ax = 0.5e0_qxknd * (x2 - x1)
bx = 0.5e0_qxknd * (x2 + x1)
...
!   For input full precision value A, this routine returns a DP approximation to log10 (A).
...
da = dble (a)
if (da .eq. 0.e0_qxknd) then
  dplog10q = -999999.e0_qxknd
else
  dplog10q = log10 (abs (da)) + ia * log10 (2.e0_qxknd)
endif
