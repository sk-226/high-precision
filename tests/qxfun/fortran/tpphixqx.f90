...
!   special functions are required here -- only the quad type kind).
...
!           default = 25. Run will abort if this is exceeded.

use qxmodule
implicit none
integer, parameter:: idb = 2, ipal = 1, kd = 10, kp = 3, kq = 3, n = 5, mpi = 8, &
  ndp = 34, m1 = 40, m2 = 2, nep = 3 - ndp
real (kind(0.d0)), parameter:: rb = 25.d0
real (kind(0.d0)), external:: second
complex (qxknd), external:: theta1, theta2, theta3, theta4
integer i, iq, i1, i2, j, j1, k, lcx, lmx, l1, m, n1
character(32) form32
real (kind(0.d0)) d1, d2, tm, tm0, tm1, tm2, tm3
real (qxknd) al, beta, eps, epsm, pi, qq, xx, yy
complex (qxknd) zz, c1, c2, c3, c4
real (qxknd) r(n), r2(2*n), x(n)
save
...
pi = qxpi ()
eps = 10.e0_qxknd ** nep
xx = real (kp, qxknd) / real (kd, qxknd)
yy = real (kq, qxknd) / real (kd, qxknd)
...
tm0 = second ()
qq = exp (- pi)
zz = 0.5e0_qxknd * pi * cmplx (yy, xx, qxknd)
c1 = theta1 (zz, qq, eps)
c2 = theta2 (zz, qq, eps)
c3 = theta3 (zz, qq, eps)
c4 = theta4 (zz, qq, eps)
...
6   format (/'Recovered half relation: 0 =')
    ...
    do i = 1, n
      if (r(i) .ne. 0.e0_qxknd) then
        write (6, fmt=form32) r(i)
      endif
    enddo
  endif
...
!   Output full polynomial.
  ...
  do i = 1, n1
    if (r2(i) .ne. 0.e0_qxknd) then
        write (6, fmt=form32) r2(i)
    endif
  enddo
endif
...
stop
end program tpphixqx

subroutine doublep (n, polyh, poly)
use qxmodule
implicit none
integer, intent(in):: n
real (qxknd), intent(in):: polyh(0:n/2)
real (qxknd), intent(out):: poly(-n/2:n/2)
real (qxknd) x(-n/2-1:n/2+1), y(-n/2-1:n/2+1)
integer i, j, k
...
complex (qxknd) function theta1 (zz, qq, eps)
use qxmodule
implicit none
complex (qxknd), intent(in):: zz
real (qxknd), intent(in):: qq, eps
integer k1, k2, n
real (qxknd) ds
real (qxknd) r1
complex (qxknd) t0, t1, t2, t3, t4

t0 = cmplx (0.e0_qxknd, 0.e0_qxknd, qxknd)
r1 = sqrt (sqrt (qq))
ds = -1.e0_qxknd
...
do k1 = 1, 10001, 2
  ds = - ds
  k2 = k1 ** 2
  t2 = ds * r1 ** k2 * sin (real (k1, qxknd) * zz)
  t0 = t0 + t2
...
theta1 = 2.e0_qxknd * t0
return
end function theta1

complex (qxknd) function theta2 (zz, qq, eps)
use qxmodule
implicit none
complex (qxknd), intent(in):: zz
real (qxknd), intent(in):: qq, eps
...
theta2 = 2.e0_qxknd * t0
return
end function theta2

complex (qxknd) function theta3 (zz, qq, eps)
use qxmodule
implicit none
complex (qxknd), intent(in):: zz
real (qxknd), intent(in):: qq, eps
...
do k1 = 1, 10000
  k2 = k1 ** 2
  t2 = r1 ** k2 * cos (2.e0_qxknd * real (k1, qxknd) * zz)
  t0 = t0 + t2
  if (abs (t2) < eps) goto 100
enddo
...
theta3 = cmplx (1.e0_qxknd, 0.e0_qxknd, qxknd) + 2.e0_qxknd * t0
return
end function theta3

complex (qxknd) function theta4 (zz, qq, eps)
use qxmodule
implicit none
complex (qxknd), intent(in):: zz
real (qxknd), intent(in):: qq, eps
...
do k1 = 1, 10000
  ds = - ds
  k2 = k1 ** 2
  t2 = ds * r1 ** k2 * cos (2.e0_qxknd * real (k1, qxknd) * zz)
  t0 = t0 + t2
  if (abs (t2) < eps) goto 100
enddo
...
theta4 = cmplx (1.e0_qxknd, 0.e0_qxknd, qxknd) + 2.e0_qxknd * t0
return
end function theta4
...
!     eps   QXR     Tolerance for full precision relation detection. 
!     x     QXR     Input dd_real vector.
!     iq    int     Output flag: 0 (unsuccessful) or 1 (successful).
!     r     QXR     Output integer relation vector, if successful.
...
real (kind(0.d0)), intent(in):: rb
real (qxknd), intent(in):: eps, x(n)
integer, intent(out):: iq
real (qxknd), intent(out):: r(n)
integer, parameter:: ipi = 25, ipm = 100, itm = 100000, nsq = 8
real (kind(0.d0)), external:: dplog10, second
real (qxknd), external:: bound
integer i, imq, it, izm, j, j1, n1, n2, n3, n4
real (kind(0.d0)) d1, d2, d3, d4, tm0, tm1, times(2)
real (qxknd) b(n,n), h(n,n), s(n), syq(n,nsq), y(n), rn, t1, t2, t3, t4
...
implicit none
integer, intent(in):: n
real (qxknd), intent(in):: y(n)
real (qxknd), intent(out):: y1, y2
integer i
real (qxknd) t1, t2, t3
...
real (qxknd), intent(in):: x(n)
real (qxknd), intent(out):: b(n,n), h(n,n), syq(n,nsq), y(n)
integer i, i1, j
real (qxknd) d1
real (qxknd) s(n), t1, t2
...
!   This performs one iteration of the PSLQM algorithm using full precision.
...
real (qxknd), intent(in):: eps
real (qxknd), intent(inout):: b(n,n), h(n,n), syq(n,nsq)
real (qxknd), intent(out):: y(n)
integer, intent(out):: izm
integer, parameter:: ntl = 4
integer i, ii, ij, im, im1, j, j1, j2, k, mpr, mq, n1
real (kind(0.d0)) d1, d2
integer ip(n), ir(n), is(n)
real (qxknd) q(n), t(n,n)
real (qxknd) gam, t1, t2, t3, t4, teps

teps = 2.e0_qxknd ** ntl * eps
izm = 0
mpr = nint (0.4e0_qxknd * n)
gam = sqrt (4.e0_qxknd / 3.e0_qxknd)
...
real (qxknd) function bound (n, h)

!   This computes the norm bound using full precision.

use qxmodule
implicit none
integer, intent(in):: n
real (qxknd), intent(in):: h(n,n)
integer i
real (qxknd) t1, t2
...
da = dble (a)
if (da == 0.e0_qxknd) then
  dplog10 = -999999.e0_qxknd
else
...
!   This outputs the full precision matrix A as a DP matrix.

use qxmodule
implicit none
integer, intent(in):: n1, n2
real (qxknd), intent(in):: a(n1,n2)
integer i, j, ix(n2)
real (kind(0.d0)) dx(n2)
...
!   This outputs the full precision matrix A.  It may be used in place of calls
!   to matoutmd in the code above if greater accuracy is desired in debug output.
...
!   This routine sorts the entries of the N-long full precision vector A into ascending
!   order using the quicksort algorithm.  The permutation vector that would sort the
!   vector is returned in IP.
!   Input: n, a.
!   Output: ip.
