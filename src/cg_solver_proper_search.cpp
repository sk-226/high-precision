#include "algorithms/conjugate_gradient_proper_search.hpp"
...
int solveCG(const SolverConfig& config) {
    ...
    auto result = algorithms::proper_search::conjugateGradient<T>(
    ...
    if (!config.export_csv_file.empty()) {
        ...
        bool csv_ok = io::CsvExporter::append_row(
            csv_path, config.matrix_name, n, A.nonZeros(), "cg",
            config.tolerance, max_iterations, "ones", config.precision_level,
            "none", "", "CG (none)", 0.0, result.converged,
