#include "algorithms/ssor_pcr.hpp"
...
SolverConfig parseCommandLine(int argc, char** argv) {
    ...
    const auto next_token = [&argc, &argv](int& i) -> std::string_view {
        if (++i >= argc) {
            ...
            throw std::runtime_error(
                "Missing value after option: " + std::string(argv[i - 1]) +
                " (try --help for usage)");
        }
    ...
    const auto parse_max_iter =
        [&parse_double_strict](
            std::string_view token) -> std::variant<int, double> {
        int fixed_max_iter{};
        const char* first = token.data();
        const char* last = first + token.size();
        auto [ptr, error_code] =
            std::from_chars(first, last, fixed_max_iter, 10);
        if (error_code == std::errc{} && ptr == last) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--omega") {
            config.omega = parse_double_strict(next_token(i));
            continue;
        }
...
void printUsage(const char* program_name) {
    ...
    std::cout
        << "                        - Integer: absolute number of iterations\n";
    std::cout << "                        - Float: coefficient * matrix_size "
                 "(default: 2.0)\n";
    std::cout
        << "  --omega VALUE         SSOR relaxation parameter (default: 1.0)\n";
    std::cout << "  --input-dir PATH      Input directory path (default: "
                 "/work/inputs)\n";
    std::cout << "  --export-mat FILE     Export convergence data to MATLAB "
                 ".mat file\n";
    std::cout << "  --export-csv FILE     Append final metrics to CSV (example "
                 "schema)\n";
    std::cout << "  --help, -h            Show this help message\n\n";
    std::cout << "Examples:\n";
    std::cout << "  " << program_name
              << " --matrix nos5 --precision qx --tol 1e-15 --omega 1.2\n";
    std::cout << "  " << program_name
              << " --matrix nos7 --precision dq --max-iter 1000 --omega 1.0\n";
    std::cout << "  " << program_name
...
template <typename T>
int solveCR(const SolverConfig& config) {
    ...
    // search matrix in the input directory (check the structure of the input
    // directory) CHECK: Download the matrix with
    // https://github.com/sk-226/ssdownload
    std::filesystem::path matrix_dir =
        std::filesystem::path(config.input_dir) / config.matrix_name;
    std::filesystem::path matrix_path =
        matrix_dir / (matrix_dir.filename().string() + ".mtx");
    std::cout << "Loading matrix: " << matrix_path.string()
              << " (precision: " << Traits::name() << ")" << '\n';
    ...
    std::cout << "\nStarting SSOR-PCR iterations...\n";
    std::cout << "Omega (SSOR parameter): " << config.omega << '\n';

    auto result = algorithms::ssor_pcr<T>(A, b, x, x_true, max_iterations,
...
#else
        std::cerr << "Warning: MATLAB export not available - built without "
                     "matio-cpp support."
                  << '\n';
#endif
    }
    ...
    if (!config.export_csv_file.empty()) {
        ...
        std::ostringstream omega_str;
        omega_str << "omega=" << config.omega;
        bool csv_ok = io::CsvExporter::append_row(
            csv_path, config.matrix_name, n, A.nonZeros(), "ssor_pcr",
            config.tolerance, max_iterations, "ones", config.precision_level,
            "ssor", omega_str.str(), "PCR (SSOR)", config.omega,
            result.converged, result.iterations_performed, result.true_relres_2,
            result.final_residual_norm,
            result.hist_relerr_2.empty()
                ? std::numeric_limits<double>::quiet_NaN()
                : result.hist_relerr_2.back(),
            result.hist_relerr_A.empty()
                ? std::numeric_limits<double>::quiet_NaN()
                : result.hist_relerr_A.back(),
            result.computation_time,
            result.converged ? std::string("reached_tol")
...
int main(int argc, char** argv) {
    try {
        ...
        if (error_msg.find("argument") != std::string::npos ||
