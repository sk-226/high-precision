#include "algorithms/conjugate_residual.hpp"
...
void printUsage(const char* program_name) {
    ...
    std::cout << "  " << program_name
              << " --matrix nos5 --precision qx --tol 1e-15\n";
    std::cout << "  " << program_name
              << " --matrix nos7 --precision dq --max-iter 1000\n";
    std::cout << "  " << program_name
              << " --matrix test --precision dd --max-iter 2.5\n";
    std::cout << "  " << program_name
              << " --matrix nos5 --precision double --tol 1e-10\n";
    std::cout << "  " << program_name
...
int solveCR(const SolverConfig& config) {
    ...
    std::cout << "\nStarting CR iterations...\n";

    auto result = algorithms::conjugateResidual<T>(
        A, b, x, x_true, max_iterations, config.tolerance);
    ...
    if (!config.export_csv_file.empty()) {
        ...
        bool csv_ok = io::CsvExporter::append_row(
            csv_path, config.matrix_name, n, A.nonZeros(), "cr",
            config.tolerance, max_iterations, "ones", config.precision_level,
            "none", "", "CR (none)", 0.0, result.converged,
            result.iterations_performed, result.true_relres_2,
