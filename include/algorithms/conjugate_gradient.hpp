...
struct CGResult {
    ...
    std::optional<std::vector<double>> hist_res_orthogonality;  ///< Residual orthogonality
    std::optional<std::vector<double>> hist_search_direction_A_orthogonality;  ///< Search direction
                                                                               /// A-orthogonality
    ...
    std::optional<int> proper_search_lag{0};  ///< Proper-search lag (0 means unused)
};
...
template <typename T>
void print_results(const CGResult<T>& result,
    ...
    std::cout << "Precision: " << result.precision_name << " ("
              << bailey::PrecisionTraits<T>::decimal_digits() << " digits)"
              << std::endl;
    std::cout << "========================== " << std::endl;

    if (result.converged) {
        std::cout << "Converged! (iter = " << result.iterations_performed << ")"
                  << std::endl;
    } else {
        std::cout << "NOT converged. (max_iter = "
                  << result.iterations_performed << ")" << std::endl;
    }

    std::cout << "# Iter.: " << result.iterations_performed << std::endl;
    std::cout << std::fixed << std::setprecision(3)
              << "Time[s]: " << result.computation_time << std::endl;
    std::cout << std::scientific << std::setprecision(2);

    // Display final convergence metrics
    int final_idx = result.iterations_performed;
    std::cout << "Relres_2norm = " << result.hist_relres_2[final_idx]
              << std::endl;
    std::cout << "True_Relres_2norm = " << result.true_relres_2 << std::endl;
    std::cout << "Relerr_2norm = " << result.hist_relerr_2[final_idx]
              << std::endl;
    std::cout << "Relerr_Anorm = " << result.hist_relerr_A[final_idx]
              << std::endl;
    std::cout << "========================== " << std::endl;
    std::cout << std::endl;
}
...
inline int resolve_max_iterations(const std::variant<int, double>& spec,
                                  int matrix_size) {
